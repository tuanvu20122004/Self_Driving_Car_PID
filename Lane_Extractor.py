...
def MaskExtract():
    ...
    cv2.imshow("[[LAN MAU TRANG] mask]",dst)
    cv2.imshow("[[LAN MAU VANG] mask_Y]",dst_Y)
# def MaskExtract():
    # global HLS, src
    # if isinstance(HLS, int) or HLS is None or isinstance(HLS, int) and HLS == 0:
        # return  
   
    # mask = clr_segment(HLS, (Hue_Low, Lit_Low, Sat_Low), (255, 255, 255))  
    # mask_Y = clr_segment(HLS, (Hue_Low_Y, Lit_Low_Y, Sat_Low_Y), (Hue_High_Y, 255, 255))  
    
    # mask_ = mask != 0
    # dst = src * (mask_[:, :, None].astype(src.dtype))  
    #cv2.imshow("[[LAN U?NG GI?A] mask]", dst)  
    #cv2.imshow("[[LAN MAU VANG] mask_Y]", dst_Y)  


    if(config.clr_segmentation_tuning):
        cv2.namedWindow("[[LAN MAU TRANG] mask]",cv2.WINDOW_NORMAL)
        cv2.namedWindow("[[LAN MAU VANG] mask_Y]",cv2.WINDOW_NORMAL)

        cv2.createTrackbar("Hue_L","[[LAN MAU TRANG] mask]",Hue_Low,255,OnHueLowChange)
        cv2.createTrackbar("Lit_L","[[LAN MAU TRANG] mask]",Lit_Low,255,OnLitLowChange)
        cv2.createTrackbar("Sat_L","[[LAN MAU TRANG] mask]",Sat_Low,255,OnSatLowChange)

        cv2.createTrackbar("Hue_L","[[LAN MAU VANG] mask_Y]",Hue_Low_Y,255,OnHueLowChange_Y)
        cv2.createTrackbar("Hue_H","[[LAN MAU VANG] mask_Y]",Hue_High_Y,255,OnHueHighChange_Y)
        cv2.createTrackbar("Lit_L","[[LAN MAU VANG] mask_Y]",Lit_Low_Y,255,OnLitLowChange_Y)
        cv2.createTrackbar("Sat_L","[[LAN MAU VANG] mask_Y]",Sat_Low_Y,255,OnSatLowChange_Y)
